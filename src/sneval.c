...
sn_error_t sn_program_run_main2(sn_program_t *prog, sn_value_t *arg, sn_value_t *value_out)
{
    *value_out = sn_null;
    sn_error_t status = SN_SUCCESS;
    sn_env_t *env = sn_env_create(&prog->globals, NULL);
...
sn_error_t sn_expr_eval_while(sn_eval_t *e)
{
    ...
    *e->val_out = sn_null;
    ...
    while (true) {
        sn_error_t status = sn_expr_eval(cond_expr, e->env, e->val_out);
        if (status != SN_SUCCESS) {
            return status;
        }
